...
function buildSvg(title: string, tags: string[]): string {
  ...
  const tagPills = tags
    ...
    .map((tag, i) => {
      const x = 52 + i * 150;
      const display = tag.length > 14 ? tag.slice(0, 13) + "…" : tag;
      return `<g>
        <rect x="${x}" y="403" width="138" height="28" rx="14" fill="rgba(255,255,255,0.2)"/>
        <text x="${x + 69}" y="422" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="13" font-family="sans-serif">${esc(display)}</text>
      </g>`;
    })
    .join("");
  ...
  <text x="52" y="68" fill="rgba(255,255,255,0.55)" font-size="14" font-family="sans-serif" font-weight="600" letter-spacing="2.5">PROJECT</text>
